...
> **Distinction from the harness built-in `/loop`:** The built-in `/loop` is a Claude Code harness primitive that keeps a single agent turn running. `/agentic-framework-beta:loop` is this framework's OPT-IN orchestrator-level control loop: it is goal-driven, ticket-driven, and namespaced to the `agentic-framework-beta` plugin. The two operate at different abstraction levels and must not be confused.
...
### Step 1 — Acquire the session lock (TASK-061)
...
### Step 3 — Release the lock (finally-style)

Always release the lock via `release()` from `src/session-lock.js` on exit, pause, or any unhandled error. This is not optional — a held lock blocks other sessions.
