...
```
/agentic-framework-beta:mode harness
/agentic-framework-beta:mode loop
```
...
- **Crash recovery**: if the autonomous loop (`/agentic-framework-beta:loop`) crashed without releasing cleanly, the bundle may be stuck at `mode: 'loop'`. Run `/agentic-framework-beta:mode harness` to reset it.
...
- The mode auto-flips during a normal `/agentic-framework-beta:loop` run: `loop` after `acquire()` succeeds, `harness` before `release()`. Manual use of this command outside the loop is only needed for recovery or testing.
