...
returns an `McpServer` with every tool registered (seven task-store tools plus
the extension tools — `kb_lookup` (TASK-106), `kb_graph_query` (TASK-168),
...
(No tool COUNT is stated here on purpose — TASK-208 fix round: a count rots on
every added/removed tool and can't be mechanically sensed without hardcoding a
number-word, which relocates the drift. `tests/mcp-tool-doc-lock.spec.js` is
the live source of truth for the actual list.)

```js
// src/mcp-server.js (shape)
export function createServer({ repoRoot }) {
  const server = new McpServer({ name: 'hivemind-tasks', version: '0.1.0' });
  // ...registerTool for every tool above, each closing over `repoRoot`...
  return server;
}
...
- `listTools()` on the client is a cheap extra assertion that every tool
  registered (assert the returned `tools` array's names cover, at minimum,
  `list_todos`, `list_ready`, `get_task`, `create_task`,
  `transition_status`, `append_comment`, `close_task`, `kb_lookup`,
  `kb_graph_query`, `mcp_build_status` — do not also assert a hardcoded
  `.length`, which rots on the next added tool) — good for an AC1-style "the
