...
// ---------------------------------------------------------------------------
// validateLocalhostUrl — TASK-068 XSS guard for the preview iframe src.
//
// Exported as a named export so tests/validate-localhost-url.spec.js can import
...
// ---------------------------------------------------------------------------
// Sentinel replaced by validateLocalhostUrl's source text during buildHtml().
const VALIDATE_FN_SENTINEL = '// INJECTED_VALIDATE_LOCALHOST_URL';

function buildHtml() {
  // Build the template, then inject the exported validateLocalhostUrl function
  // source so the browser runs the exact same code the Node tests import.
  // We use a replacement function (not a string) so no $-pattern interpretation.
  const fnSrc = validateLocalhostUrl.toString()
    // Rename: the top-level export is declared as `export function validateLocalhostUrl`
    // but inside a <script> block it must be a plain function declaration.
    // .toString() includes the function keyword and name, so we just need to
    // strip the `export ` prefix if present (esbuild may or may not strip it).
    .replace(/^export\s+/, '');
