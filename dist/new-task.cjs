...
function tasksLockPath(repoRoot) {
  return (0, import_node_path2.join)(tasksDir(repoRoot), ".mutate.lock");
}
...
`, "utf8");
      ...
      try {
        stat = (0, import_node_fs2.statSync)(lockPath);
      } catch {
        try {
          foreignEntry = (0, import_node_fs2.lstatSync)(lockPath).isSymbolicLink();
        } catch {
        }
      }
      if (foreignEntry || stat && (Date.now() - stat.mtimeMs > TASKS_LOCK_STALE_MS || isImplausiblyFuture(stat.mtimeMs))) {
        ...
        if (renamed) {
          ...
          try {
            qStat = (0, import_node_fs2.lstatSync)(quarantinePath);
          ...
          if (genuinelyStale) {
            try {
              ...
              try {
                (0, import_node_fs2.rmSync)(quarantinePath, { recursive: true, force: true });
              } catch {
              }
            }
          } else {
...
function startTasksLockHeartbeat(repoRoot, token) {
  ...
  const timer = setInterval(() => {
    try {
      ...
      (0, import_node_fs2.utimesSync)(lockPath, now, now);
    } catch {
    }
...
async function createTask({
  ...
  const { key, target, warnings } = await withTasksLock(repoRoot, async () => {
    ...
    let reserveFd;
    try {
      reserveFd = (0, import_node_fs2.openSync)(taskTarget, import_node_fs2.constants.O_CREAT | import_node_fs2.constants.O_EXCL | import_node_fs2.constants.O_WRONLY, 384);
    } catch (err) {
    ...
    const onDisk = (0, import_node_fs2.readFileSync)(taskTarget, "utf8");
    if (onDisk !== taskBytes) {
      throw new KeyCollisionError(
