...
function resourceActivations({ tier, activations }) {
...
var descriptor_default = {
  id: "design-power",
  name: "Dise\xF1o Poderoso",
  version: "0.1.0",
  core_compat: ">=1.0.0",
  trigger: {
    intake_question: "Does this project have a visual, human-facing interface that needs deliberate design work \u2014 as opposed to a pure API, CLI, backend service, or script? (This decides whether the design-power pack engages at all.)",
    activates_when: "design_heavy==yes"
  },
  ...
  resources: [
    {
      id: "frontend-design",
      kind: "plugin",
      origin: "official Claude plugin marketplace (e.g. anthropics/claude-code frontend-design plugin)",
      pin: "marketplace-latest (Wave-2; human to ratify)",
      scope: "project",
      required: "hard",
      activate_when: "always (design_heavy)",
      install: "/plugin install frontend-design@<marketplace-pin-tbd> (Wave-2, TUI-only \u2014 human to ratify exact pin before autonomous install)"
    },
    {
      id: "ui-ux-pro-max",
      kind: "skill",
      ...
      scope: "project",
      required: "soft",
      activate_when: "tier != LIGERO",
      install: "vendor via assimilate (FP-5)"
    },
    {
      id: "shadcn/ui",
      kind: "mcp",
      ...
      scope: "project",
      required: "soft",
      activate_when: "framework==react && ui_outside_canvas",
      install: "claude mcp add shadcn -- npx shadcn@4.13.0 mcp"
    },
    {
      id: "shadcn-vue",
      kind: "mcp",
      origin: "shadcn-vue CLI",
      pin: "2.7.4",
      scope: "project",
      required: "soft",
      activate_when: "framework==vue && ui_outside_canvas",
      install: "npx shadcn-vue@2.7.4 mcp init --client claude"
    },
    {
      id: "firecrawl",
      kind: "mcp",
      origin: "firecrawl-mcp (Mendable)",
      pin: "3.22.3",
      scope: "project",
      required: "soft",
      activate_when: "needs_research",
      install: "claude mcp add firecrawl -e FIRECRAWL_API_KEY=<key> -- npx -y firecrawl-mcp@3.22.3"
    },
    {
      id: "playwright",
      kind: "mcp",
      origin: "@playwright/mcp (Microsoft)",
      pin: "0.0.77",
      scope: "project",
      required: "soft",
...
var ALWAYS_ACTIVATE_SENTINEL = "always";
function resolveDesired(descriptor, profileResult) {
  const resources = Array.isArray(descriptor?.resources) ? descriptor.resources : [];
  let activations;
  const getActivations = () => {
    if (activations === void 0) activations = resourceActivations(profileResult);
    return activations;
  };
  return resources.filter((resource) => resource && (resource.activate_when === ALWAYS_ACTIVATE_SENTINEL || getActivations()[resource.id] === true));
}
...
// state/integrations-lock.schema.json
var integrations_lock_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://hivemind.local/state/integrations-lock.schema.json",
  title: "IntegrationsLock",
  description: "Reconciler lockfile for the addon-pack subsystem (docs/design/addon-packs-plan.md \xA72). Ownership is tracked as owners[] EDGES (pack-id@version strings), not a counter \u2014 the Nix reachability model. A resource is safe to remove only when owners is empty; that decision and the physical removal both belong to the applier, never to this store.",
  type: "object",
  ...
  properties: {
    ...
    resources: {
      type: "object",
      description: 'Keyed by resource id, e.g. "mcp:firecrawl", "skill:shadcn-vue".',
      additionalProperties: {
        type: "object",
        required: [
          "kind",
        ...
        properties: {
          ...
          owners: {
            ...
            description: "pack-id@version edges. Empty \u21D2 safe to remove (Nix reachability model, not a counter)."
          },
          required: { type: "string", enum: ["hard", "soft"] },
          installed_at: { type: "string", format: "date-time" },
          install_method: { type: "string", minLength: 1 },
          verified: { type: "string", enum: ["signed", "unsigned", "unknown"] }
        }
      }
    }
  }
...
// src/integrations-lock.js
var UnknownResourceIdError = class extends Error {
  constructor(id) {
...
function getValidator() {
  ...
  _validate = ajv.compile(integrations_lock_schema_default);
  return _validate;
}
...
function addOwner(lock, id, owner) {
...
function dropOwner(lock, id, owner) {
  const entry = getEntryOrThrow(lock, id);
  entry.owners = (entry.owners || []).filter((o) => o !== owner);
  return lock;
}
function isOrphaned(lock, id) {
  const entry = getEntryOrThrow(lock, id);
...
async function reconcilePack(opts = {}) {
  ...
  const lockPath = opts.lockPath || (0, import_node_path6.join)(repoRoot, DEFAULT_LOCK_FILENAME);
...
function profileResultFromFrontmatter(answers) {
  ...
  if (tier === void 0 || !p || Object.keys(p).length === 0) {
...
function aggregateDesired(activePacks, profileResult) {
  const out = [];
