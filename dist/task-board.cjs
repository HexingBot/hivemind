...
<header class="app-header">
  ...
  <span class="tagline">hivemind \xB7 agentic development framework</span>
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (tok === "--port") {
      const raw = argv[++i];
      if (raw === void 0) throw new Error("--port requires a value");
      const n = Number(raw);
...
function openBrowser(url, spawnFn = import_node_child_process.spawn) {
  try {
    let cmd, args;
