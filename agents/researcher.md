...
model: sonnet
tools: Read, Grep, Glob, WebSearch, WebFetch, Write, mcp__github__*, mcp__wisearcher-brain__*, mcp__plugin_hivemind_hivemind-tasks__kb_lookup, mcp__plugin_hivemind_hivemind-tasks__kb_graph_query
---
...
1. **Call the `kb_graph_query` MCP tool first** (`mcp__plugin_hivemind_hivemind-tasks__kb_graph_query`, TASK-168/172) — never hand-read `knowledge/graph/graph.json`; the tool is now the graph's single query surface, callable regardless of whether the brain is up. It is canonical-first-then-local: an `id` query resolves against the canonical wisearcher brain graph first when the brain is wired in (opt-in/experimental, off by default), and falls back to the deterministic local `knowledge/graph/graph.json` projection when the brain is absent (the default). Concrete arg shapes: `kb_graph_query({ id: 'task-<n>' })` for the neighbors of a ticket node (e.g. `task-168`), `kb_graph_query({ type: 'decision' })` to enumerate every decision node, `kb_graph_query({ id, type })` to post-filter an id's neighbors down to one node type. It returns `{ query, source, nodes }`, sorted by id ascending on the local path — a missing id yields empty `nodes`, never a throw. Read the referenced entries/skills at their surfaced `ref` before proceeding.
