...
1. **Graph lookup first.** Read `knowledge/graph/graph.json` directly (your tool grant has no execution seam for `src/knowledge-graph.js` — its `neighbors`/`nodesByType` functions describe the query shape below but are not callable from this seat, so do not simulate calling them). Find `nodes` of type `knowledge_entry` or `skill` whose `label` or `ref` matches key terms from the question topic, then scan `edges` for entries whose `from`/`to` reference that node's `id` to surface its connections (the same neighbor set `neighbors()` would return). Read the referenced entries/skills before proceeding. (This local graph is a projection/cache of the canonical brain graph.)
