...
> **Brain** (wisearcher) is **not yet wired** — claims about those integrations are design intent
> from PLAN.md, capped at `[INFERRED:strong]`. wisearcher and proposal-engine are real source in
...
- **Three-part fusion**: Body = `agent-framework` (orchestrator + developer/reviewer/researcher
  ...
  `wisengine` standards, **vendored in** [INFERRED:strong]; Brain = `wisearcher`, **called as an
...
- **Brain seam = MCP service** (out-of-process Python); polyglot coupling is MCP-only. The base
  ...
  **separate Python server that does not yet exist** — wisearcher is a library + CLI today
  [INFERRED:strong] (signal: `wisearcher/wisearcher/cli.py`) — so Phase 1 builds it. See [[brain-contract]].
...
- **Brain runtime is managed by hivemind** (`docker compose up` + spawn MCP) with **graceful
  fallback** to a lightweight grep KB. This matters because wisearcher has **no internal
  fallback** — Voyage/Neo4j/Qdrant are hard deps that raise on absence [INFERRED:strong] (signal:
  `wisearcher/wisearcher/embed.py`, `store/qdrant_store.py`); the fallback is entirely hivemind's
...
- **Python 3.11+** — the Brain (wisearcher), today a library + CLI; gains an MCP server in Phase 1.
  [INFERRED:strong] (signal: `wisearcher/pyproject.toml`)
- **Neo4j + Qdrant** — the canonical knowledge graph; defined in wisearcher's `docker-compose.yml`. [INFERRED:strong]
- **Voyage embeddings (`voyage-3`)** — brain embedding provider; absence raises, triggering the
  hivemind-side grep fallback. [INFERRED:strong] (signal: `wisearcher/wisearcher/embed.py`)
