...
2. **Re-run verification using the scaled gate.** Run `npm run test:since -- <base-ref>` (the base ref is the diff-range start you were given for the Developer's commits — `--changed` alone diffs against a clean `HEAD`, which is always empty once commits land, so the base ref is required) plus `npm test` (fast tier, ~2s test-execution / ~7s wall-clock — carries the fs-read sensors: parity, live-state, doc-locks — that `--changed`'s import graph cannot see) plus any affected e2e specs named in the Developer's hand-off. `test:since` is a validating wrapper (`scripts/test-since.mjs`), not a raw `vitest --changed`: it resolves your base ref with `git rev-parse --verify` and fails loudly on a bad ref instead of silently degrading — **a "No test files found" / zero-specs result is a gate FAILURE to investigate, never a green**; do not accept it as "nothing changed." A third blind-spot class (TASK-109) — a spec that loads its module under test via a runtime-computed dynamic import, e.g. `await import(pathToFileURL(...).href)` as in `tests/framework-bug-report.spec.js` — defeats both `test:changed` and `test:since` whenever the spec file itself sits outside the diff range; if the touched src file has such a spec, also run it directly (`npx vitest run <spec>`) so a stale-green does not slip through. The Developer proposes the affected-e2e list; independently assess its sufficiency — expand it or escalate to the Orchestrator if it appears under-scoped. Run `npm run test:all` only when the ticket touches test infrastructure or `tasks/schema.json`, or at release/milestone/publish points. A green Developer hand-off must reproduce as green here.
   - **UAT-completeness check — triggered by `requires_uat: true`, never by `verification_tier` and never by whether a `uat` comment happens to exist (TASK-220):** whenever the ticket carries `requires_uat: true` (see `requiresUat()` in `src/task-store.js` for how the default resolves when the field is absent), verify that (a) a `uat` comment exists on the ticket, (b) its steps cover **every** acceptance criterion, and (c) all steps are marked PASS — regardless of tier, and regardless of whether the Developer's hand-off mentions UAT at all. A missing `uat` comment is itself the HIGH finding, the same severity as incomplete AC coverage or any FAIL step — its absence is exactly what this check exists to catch, not a condition that turns the check off. `uat-only` tickets add the same check for new specs (there are none to check) in place of the regression-lock review. When `requires_uat` is `false` — including when the field is absent, which resolves to `false` — this check does not run; a ticket in that state is not required to carry a `uat` comment.
...
TASK-212 (2026-08-13 human decision) retired the `tdd` tier; `tests-after` is now the tier for all real work (implement, then regression-lock), so auditing `tests-after` tickets that touch source logic against this check would fire on nearly every real ticket — noise the team would learn to ignore, not a control. Scope narrowed accordingly: when a `uat-only` ticket's diff touches source logic, state mutation, parsing, or schema files, raise a HIGH "tier misassignment" finding — glue/config/docs work was never supposed to need that surface area. The `review_depth` rubric never overrides this check: run the tier-audit regardless of whether the depth is `light` or `full`.
